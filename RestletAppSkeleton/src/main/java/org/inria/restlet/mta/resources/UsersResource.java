...
public class UsersResource extends ServerResource
{
    ...
    // Method to handle http delete request 
    
    @Delete("json")
    public Representation removeUser(JsonRepresentation representation)
        throws Exception
    {
        JSONObject object = representation.getJsonObject();
        int userId = object.getInt("id");
        backend_.getDatabase().removeUser(userId);

        JSONObject resultObject = new JSONObject();
        resultObject.put("Alert Message: ", "Given user has been deleted");
        JsonRepresentation result = new JsonRepresentation(resultObject);
        return result;
    }
